...
RSpec.describe User, type: :model do
  ...
  describe 'バリデーション' do
    ...
    it 'password が空なら invalid（暗号化を通さず生の空のまま保存し、presence で弾く）' do
      user = User.new(valid_attrs.merge(password: ''))
      expect(user).to be_invalid
  ...
  describe 'email の正規化と一意性ポリシー（大小無視）' do
    it 'before_validation で email を小文字化・trim する' do
      user = User.new(valid_attrs.merge(email: '  ADMIN@Example.COM  '))
      user.valid?
    ...
    it '大文字小文字違いでも同一 email として扱い、2件目は invalid' do
      ...
      User.create!(valid_attrs.merge(email: existing_email))

      dup = User.new(valid_attrs.merge(email: existing_email.upcase))
  ...
  describe '#authenticate' do
    it '正しいパスワードで true、誤りで false' do
      user = User.create!(valid_attrs.merge(password: 'secret123'))
